...
func _building(b: Dictionary, loc_id: String) -> void:
	...
	var fp := Rect2(r.position * PX, r.size * PX)
	...
	_box(_root, Vector3(fp.size.x, WALL_H, fp.size.y), centre + Vector3(0, WALL_H / 2, 0), wall)
	...
	ri.rotation.y = PI / 2
	_root.add_child(ri)
	var door := _v(Geo.vec(b["door"]))
	var open: bool = Conditions.check(b.get("unlock", {}), Game.state, loc_id)
	_box(_root, Vector3(0.9, 1.5, 0.08), Vector3(door.x, 0.75, fp.end.y + 0.04), _mat(Color("#6b3f24") if open else Color("#4a4440")))
	...
	for wx in [fp.position.x + fp.size.x * 0.2, fp.end.x - fp.size.x * 0.2]:
		if absf(wx - door.x) > 0.7:
			_box(_root, Vector3(0.6, 0.6, 0.06), Vector3(wx, 1.4, fp.end.y + 0.03), win)
	_label(_root, str(b.get("label", b.get("prompt", ""))), centre + Vector3(0, WALL_H + 1.6, fp.size.y / 2), 56, Color.WHITE)
...
func _exit(e: Dictionary) -> void:
	...
	_box(_root, Vector3(1.0, 0.04, 0.5), p + Vector3(0, 0.02, 0), _mat(Color("#6b4a32")))
...
func _sync(delta: float) -> void:
	...
	_player.position = _v(p.position)
	var f: Vector2 = p.facing
	_player_body.rotation.y = lerp_angle(_player_body.rotation.y, atan2(f.x, f.y), 0.3)
	var signals: Dictionary = Game.world_signals()
	for npc_id in _npcs:
		var kind := str(signals.get(npc_id, ""))
		var lab: Label3D = _signals[npc_id]
	...
	_focus.visible = not focused.is_empty()
	if _focus.visible:
	...
	var dist := 13.0 if interior else 16.0
	var want := target + Vector3(0, sin(pitch), cos(pitch)) * dist
	_cam.position = _cam.position.lerp(want, clampf(delta * 5.0, 0.0, 1.0))
	_cam.look_at(_cam.position - Vector3(0, sin(pitch), cos(pitch)), Vector3.UP)
...
func _flat(r: Rect2, h: float, m: Material) -> void:
	_box(_root, Vector3(r.size.x * PX, h, r.size.y * PX), _v(r.get_center()) + Vector3(0, h / 2.0, 0), m)

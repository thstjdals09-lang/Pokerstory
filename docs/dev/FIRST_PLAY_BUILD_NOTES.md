...
| 항목 | 내용 |
|---|---|
| 기준 | [First Play Spec v0.2](../design/Project20_First_Play_Spec_v0.2.md) + [Design Correction 02](../design/Project20_Design_Correction_02.md) + [Economy v0.2.1](../design/Project20_Economy_v0.2.1.md) |
| 엔진 | Godot 4.7.2 stable (GDScript만 사용, 렌더러 GL Compatibility) |
| 대상 | Windows PC, 키보드 + 마우스 |
| 작성 | Claude (개발 담당), 최종 갱신 2026-09-25 |
| 상태 | 수정 지시 02 반영 완료, 자동 검증 통과. Gate 2(플레이어 시각 검수) 대기 |
...
```
새 게임 (칩 100, 낮)
 ├ 광장: 루미(낮), 아르바이트 게시판 → 카드·칩 5개 줍기 → +10 (반복 가능)
...
│  ├─ poker.json            룰, 경제(Economy v0.2.1), 능력, 대사
│  ├─ quests.json           일회성 의뢰
│  ├─ jobs.json             반복 아르바이트 (보상, 개수, 후보 지점)
│  └─ goals.json            HUD 현재 목표
├─ scripts/core/            노드에 의존하지 않는 규칙 (단위 테스트 대상)
│  ├─ card, deck, hand_evaluator, poker_ai, poker_match
│  ├─ poker_economy         참가금 차감, 정산, 포기, 강제 종료 환불
│  ├─ quest_book            의뢰 상태, 1회 보상
│  ├─ plaza_job             아르바이트 1회분 (지점별 1회, 완료 시 1회 지급)
│  ├─ game_state            저장 대상 상태 (v2), 칩 원장
│  ├─ save_system           JSON 단일 슬롯, 버전, v1 → v2 변환
...
|---|---|
| 단위 테스트 | **54개 통과** / 0개 실패 |
| E2E `win` / `draw` / `lose` | 127 / 127 / 125개 검증 통과 |
| E2E `broke` (파산 → 참가 제한 → 아르바이트 → 재참가) | 111개 검증 통과 |
| E2E `resume_a` → `resume_b` → `resume_c` (프로세스 3개) | 8 / 19 / 26개 검증 통과 |
| E2E `migrate` (v1 저장 파일) | 4개 검증 통과 |
| E2E `reject` (읽을 수 없는 저장 파일) | 7개 검증 통과 |
| 창 모드 E2E `win`, `broke` + 스크린샷 | 127 / 111개 검증 통과 |
...
## 5. 저장 형식 (save_version 2)

저장 위치: `user://save_slot_1.json` (Windows: `%APPDATA%\Godot\app_userdata\Pokerstory\`)

| 키 | 내용 |
|---|---|
| `save_version` | 2. v1은 자동 변환(잔액 유지). 더 새 버전이나 손상된 파일은 거부하고 파일을 건드리지 않음 |
| `chips_balance`, `chips_ledger[]` | 잔액(음수 불가)과 원장 `{seq, delta, reason, balance}` |
| `pending_poker_stake` | 정산 전 참가금. 판 진행 중에만 0이 아님. 불러올 때 남아 있으면 1회 환불 |
...
**저장 시점:** 새 게임, 장소 이동, 대화 플래그, 시간 변경, 참가금 차감, 정산·포기, 의뢰 수락·완료, 아르바이트 완료, 구매, 배치, 메뉴 저장, 타이틀 이동, 종료.

---

## 6. 명세와의 차이

v0.2 첫 빌드의 불일치 4건(패배 보상, 생활 의뢰 미구현, 루미 중복 배치, 시간 초기화)은 수정했습니다. 자세한 내용은 [Design Correction 02](../design/Project20_Design_Correction_02.md)에 있습니다. 개발 측이 정해서 기획 확인이 필요한 처리(D1~D7)도 같은 문서 5절에 있습니다.
...
- 경제 밸런스(예: 100칩으로 시작하면 첫 방문에 바로 등불을 살 수 있음)는 승인 수치대로 구현했고, 체감 검증은 하지 않았습니다.
...
1. 새 게임 → HUD 칩 100, 낮
2. 광장의 루미와 인사 → 아르바이트 게시판 → 카드·칩 5개를 걸어 다니며 줍기 → +10
3. 세라의 잡화점 → 의뢰 수락 → HUD 목표가 꾸러미 전달로 바뀌는지
4. 카드룸 문(낮) → "저녁까지 기다리기" → 저녁 카드룸에 루미와 모아
5. 루미에게 꾸러미 전달 → +30. 다시 말 걸면 전달 선택지가 없는지
